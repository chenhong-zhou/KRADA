...
class SYNTHIA_Openset_pseudo_DataSet(data.Dataset):
    def __init__(self, root, list_path, pseudo_root, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255):
      ...
      #                        8: 13, 18: 14, 19: 15, 20: 16, 12: 17, 11: 18}
        ...
        for name in self.img_ids:
         ...
         #   label_file = osp.join(self.root, "GT/LABELS16_qudiao_unknown_3lei/%s" % name)
            pseudo_label_file = osp.join(self.pseudo_root, "%s" % name)
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            label = np.asarray(imageio.imread(datafiles["pseudo_label"], format='PNG-FI'))#[:,:,0]  # uint16
          #  print('label: ', label)
        ...
        #    print('label before', np.asarray(label, np.float32).shape)
     ...
     #   print('label.shape',label.shape)
        return image.copy(), label.copy(), np.array(size), np.array(size), name  #label_copy.copy()


if __name__ == '__main__':
    dst = SYNTHIA_Openset_pseudo_DataSet("./data", is_transform=True)
