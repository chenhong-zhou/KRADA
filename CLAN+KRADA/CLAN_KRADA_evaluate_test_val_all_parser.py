...
def get_arguments():
    ...
    parser.add_argument("--file-dir",'-f', type=str, 
                        help="Path to the directory containing the snapshot.")
    parser.add_argument("--start-num",'-s', type=int, 
                        help="start_num of the snapshot.")
...
def compute_mIoU(pred_imgs, gt_imgs, json_path):
    ...
    for ind in range(len(gt_imgs)):
        ...
        unk_pred = np.where(pred_f_ignored == 13)
        ...
        hist += fast_hist(label_f_ignored, pred_f_ignored, num_classes)
...
print('file_dir, start_num: ', file_dir, start_num)

for i in range(start_num, 51):

    model_path = './snapshots/' + file_dir + '/syn_{0:d}.pth'.format(i*2000)
    
    print('model_path: ', model_path)
  
    filename_prob1 = './snapshots/' + file_dir + '/val_p1.txt' 
    ...
    with torch.no_grad():
        for index, batch in enumerate(testloader):
            ...
            output1 = interp(output1).data[0]
            output2 = interp(output2).data[0]
            ...
            unk_all = torch.ones_like(prob_ind1)*13
